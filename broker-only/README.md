...
| `backend/tests/` | 174 tests: isolation between brokers (RLS), de-duplication, resolver, status rules, matching, visits and offline sync, marketplace, uploads, HTTP journeys, WebSocket |
| `tools/` | Pin-check page for the pilot broker; building-layout spreadsheet template |
| `mobile/` | Expo (React Native) broker, field-staff and owner app for Android, iOS and web, with an offline mode and an in-app demo |
