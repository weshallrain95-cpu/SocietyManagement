...
| INV-20 | **Two flat lists.** *All flats*: everything the broker added or uploaded. *Available now*: the flats the broker is offering right now, picked by them from All flats (one at a time or several at once); on-hold flats stay in it. A flat added by hand asks "Offer it now?" (default yes); uploaded flats go to All flats only. Rented out / sold / off market drops a flat out of Available now automatically; making it available again reports it available (unconfirmed until the owner says yes). Only Available now is matched to customers, shared with fellow brokers and sent in customer updates | P0 | Per broker only: another broker holding the same flat is unaffected |
