...
| INV-08 | Owner invite → broker acceptance creates the listing pre-filled from master | P1 | Owner-invited listings carry an "Owner-appointed" badge |
...
| OWN-02 | **Invite brokers** to represent the unit (one or many). Invitation carries preferred terms (expected rent, deposit, availability, house rules) | P1 | Broker accepts or declines; accepted → listing created (INV-08) |
| OWN-03 | Owner activity tracker: per broker, visits conducted, customer feedback summary, proposals/offers received | P1 | Data from linked broker listings; owner-visible events only |
| OWN-04 | Owner confirms status changes (STAT-03/05) and receives visit notices (VISIT-05) | P0 (via link) / P1 (in app) | — |
| OWN-05 | Owner uploads photos/videos and edits owner-authoritative attributes | P1 | Owner edits take precedence (Data Model §6) |
| OWN-06 | Owner revokes a broker's representation (the listing is marked "Owner withdrew") | P1 | Broker notified; listing becomes read-only history |
